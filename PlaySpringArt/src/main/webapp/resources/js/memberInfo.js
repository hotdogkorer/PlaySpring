function nicknameCheck(id){
    ...
    $.ajax({
    	...
    	success:function(args){ //3 success 성공적으로 보내준경우 익명함수
    		if(args=='true'){
	    		  console.log(args);
	    		  $('#nickcheck').html('<font color =\"red\">사용 중인 닉네임 입니다</font>');
	    	  }else{
	    		  console.log(args);
	    		  $('#nickcheck').html('<font color ="blue">사용 가능한 닉네임 입니다</font>');
	    	  }
		 }
    })  	   
}
	...
	// alert('이메일 검사'+email);
	 ...
	 $.ajax({
	    	...
	    	success:function(args){ //3 success 성공적으로 보내준경우 익명함수
	    	  if(args=='true'){
	    		  console.log(args);
	    		  $('#emailcheck').html('<font color =\"red\">사용 중인 이메일 입니다</font>');
	    	  }else{
	    		  console.log(args);
	    		  $('#emailcheck').html('<font color ="blue">사용 가능한 이메일 입니다</font>');
	    	  }
